...
function EnemyBullet:load(_world)
    world = _world
    wallHitSound = love.audio.newSource("audio/hit_wall.wav", "static")
    wallHitSound:setVolume(0.6)
end

function EnemyBullet.new(x, y, dirX, dirY, rot, damage)
    local instance = setmetatable({}, EnemyBullet)
    instance.x = x
    instance.y = y
    instance.dirX = dirX
    instance.dirY = dirY
    ...
    instance.collider:setCollisionClass("enemyBullet")
    instance.collider:setObject(instance)
    table.insert(ActiveBullets, instance)
end

function EnemyBullet:remove()
...
function EnemyBullet.removeAll()
    for i, instance in ipairs(ActiveBullets) do
        instance.collider:destroy()
    end
    ActiveBullets = {}
...
function EnemyBullet:hitDetection(dt)
    if self.collider:enter('player') then
        local collisionData = self.collider:getEnterCollisionData('player')
        local player = collisionData.collider:getObject()
        player:damage()
        self:remove()
    end
...
function EnemyBullet:updateAll(dt)
    for i, instance in ipairs(ActiveBullets) do
        instance:update(dt)
